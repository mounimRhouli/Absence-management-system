...
if (isset($_POST['viewTimetable'])) {
    $selectedDate = $_POST['viewDate'];
    $selectedClassArm = $_POST['classArmId'];

    // Corrected Query to fetch timetable entries along with teacher information
    $query = "SELECT st.startTime, st.endTime, st.sessionName, st.type, CONCAT(ct.firstName, ' ', ct.lastName) AS teacher, st.day_name
              FROM timetable t
              ...
              WHERE t.Start_Date <= '$selectedDate' AND t.End_Date >= '$selectedDate' 
              AND t.tblclassarms_id = '$selectedClassArm'";
    ...
    // Get the class/arm name for the selected class/arm ID
    $className = '';
    $classArmName = '';
    $classArmQuery = mysqli_query($conn, "SELECT tblclass.className, tblclassarms.classArmName FROM tblclassarms INNER JOIN tblclass ON tblclassarms.classId = tblclass.Id WHERE tblclassarms.Id = '$selectedClassArm'");
    if ($classArmRow = mysqli_fetch_assoc($classArmQuery)) {
        $className = $classArmRow['className'];
                        ...
                        <div class="col-lg-12">
                            ...
                            <div class="card mb-4">
                                ...
                                <div class="card-body">
                                    <form method="post">
                                        <div class="form-group row mb-3">
                                            <div class="col-xl-4">
                                                ...
                                                <input type="date" class="form-control" name="viewDate" id="viewDate" required>
                                            </div>
                                            <div class="col-xl-4">
                                                ...
                                                <select required name="classArmId" class="form-control">
                                                    ...
                                                    while ($classRow = mysqli_fetch_assoc($classQuery)) {
                                                        ...
                                                        while ($armRow = mysqli_fetch_assoc($armQuery)) {
                                                            $selected = isset($_POST['classArmId']) && $_POST['classArmId'] == $armRow['Id'] ? 'selected' : '';
                                    ...
                                    <?php if (isset($_POST['viewTimetable']) && !empty($timetableEntries)) : ?>
                                        <h5>Timetable for <?php echo $selectedDate . ', ' . $className . '/' . $classArmName; ?></h5>
                                    ...
                                    <?php elseif (isset($_POST['viewTimetable']) && empty($timetableEntries)) : ?>
                                        <div class="alert alert-info">No timetable entries found for the selected date and class/arm.</div>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </div>
